...
export function OrderPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => ordersPublicApi.getByPublicId(publicId),
    enabled: !!publicId,
  });
  ...
  if (error || !data) {
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-semibold text-gray-600 mb-2">Pedido no encontrado</h2>
        <p className="text-gray-400">El pedido que buscas no existe o fue eliminado.</p>
      </div>
    );
  }

  const { order, whatsappLink } = data;
